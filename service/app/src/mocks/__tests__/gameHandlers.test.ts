...
describe('Game API Handlers', () => {
  ...
  describe('POST /games/:gameId/plays', () => {
    ...
    it('존재하지 않는 게임 ID로 요청 시 404를 반환해야 한다', async () => {
      const response = await testFetchClient.fetch('/games/non-existent-id/plays', {
        method: 'POST',
        headers: {
          'Cookie': mockSessionCookie,
        },
  ...
  describe('POST /games/:gameId/share', () => {
    it('게임을 공유할 수 있어야 한다', async () => {
      ...
      const response = await testFetchClient.fetch('/games/share-test-game/share', {
        method: 'POST',
        headers: {
          'Cookie': mockSessionCookie,
        },
  ...
  describe('POST /games/:gameId/unshare', () => {
    it('게임 공유를 해제할 수 있어야 한다', async () => {
      ...
      const response = await testFetchClient.fetch('/games/unshare-test-game/unshare', {
        method: 'POST',
        headers: {
          'Cookie': mockSessionCookie,
        },
  ...
  describe('POST /games/:gameId/uploads/urls', () => {
    it('기존 게임용 presigned URL을 생성할 수 있어야 한다', async () => {
      ...
      const response = await testFetchClient.fetch('/games/existing-game-id/uploads/urls', {
        method: 'POST',
        headers: {
          'Cookie': mockSessionCookie,
          'Content-Type': 'application/json',
