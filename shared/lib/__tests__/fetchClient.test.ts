...
describe("FetchClient with GitHub API", () => {
  ...
  describe("Request Interceptors", () => {
    ...
    it("요청 인터셉터 에러를 처리할 수 있어야 한다", async () => {
      ...
      await expect(fetchClient.fetch("/users/octocat")).rejects.toThrow(
        "Handled error",
      )
    })
      ...
      fetchClient.addResponseInterceptor(
        ...
        async (error) => {
          return Promise.reject(
            new Error(`Intercepted: ${(error as Error).message}`),
          )
        },
      )

      await expect(
        fetchClient.fetch("/repos/nonexistent/repo"),
      ).rejects.toThrow("Intercepted: Not found")
    })
  ...
  describe("Timeout Handling", () => {
    it("타임아웃 설정이 작동해야 한다", async () => {
      ...
      await expect(timeoutClient.fetch("/delay/1")).rejects.toThrow()
    })
  ...
  describe("Complex Scenarios", () => {
    ...
    it("여러 API 엔드포인트를 순차적으로 호출할 수 있어야 한다", async () => {
      ...
      const reposResponse = await fetchClient.fetch(
        `/users/${userData.login}/repos?per_page=5`,
      )
