...
- state variant(default,hover,active)는 css 가상선택자로 구현이 되어있습니다. props로 주입하지 않아도 됩니다
- disabled 상태는 직접 주입해야 합니다(HTML 속성으로 주입하면 스타일이 적용됩니다)
...
- asChild prop을 통해 다른 엘리먼트를 버튼 스타일로 렌더링할 수 있습니다
  ex)Next.js Link 컴포넌트를 버튼 스타일로 렌더링할 수 있습니다
